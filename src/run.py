...
def get_parser():
    ...
    parser.add_argument('--mask_value', type=int, default=2, help='padding') # different value in CLModel
    ...
    parser.add_argument('--batch_size', type=int, default=32, metavar='BS', help='batch size') # 64
...
if __name__ == '__main__':
    ...
    tokenizer.add_tokens(["<mask>", "[VIS]", "[AUD]"])
    ...
    if args.disable_two_stage_training:
        ...
        with torch.no_grad():
            ...
            for batch_id, batch in enumerate(train_loader):
                input_ids, label, vis_ids, aud_ids = batch
       
                input_orig = (input_ids, vis_ids, aud_ids)
                input_aug = None
                input_ids, vis_ids, aud_ids = input_orig[0].to(device), input_orig[1].to(device), input_orig[2].to(device)
                label = label.to(device)
                if args.fp16:
                    with torch.autocast(device_type="cuda" if args.cuda else "cpu"):
                        log_prob, masked_mapped_output, masked_outputs, anchor_scores = model(input_ids, vis_ids, aud_ids, return_mask_output=True) 
                else:
                    log_prob, masked_mapped_output, masked_outputs, anchor_scores = model(input_ids, vis_ids, aud_ids, return_mask_output=True)
